...
#include "ui_zttlcontroldialog.h"
...
std::list<std::string> ZTtlControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTtlControlDialog::listMints;

ZTtlControlDialog::ZTtlControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTtlControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTtlControlDialog::~ZTtlControlDialog()
{
    delete ui;
}

void ZTtlControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTtlControlDialog::updateList()
{
...
void ZTtlControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTtlControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTtl_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTtlControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTtlControlDialog::GetSelectedMints()
{
...
void ZTtlControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
