                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTTL ought to be enough for anybody." - Bill Gates, 2017
    ui->zTTLpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTTLSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTTL->setEnabled(false);
        ui->pushButtonMintzTTL->setToolTip(tr("zTTL is currently disabled due to maintenance."));

        ui->pushButtonSpendzTTL->setEnabled(false);
        ui->pushButtonSpendzTTL->setToolTip(tr("zTTL is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTTLpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTTL_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTTL is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTTL...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTTL in ") +
...
void PrivacyDialog::on_pushButtonSpendzTTL_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTTL is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTTL
        sendzTTL();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTTL
    sendzTTL();
}

void PrivacyDialog::on_pushButtonZTtlControl_clicked()
{
    ZTtlControlDialog* zTtlControl = new ZTtlControlDialog(this);
    zTtlControl->setModel(walletModel);
    zTtlControl->exec();
}

void PrivacyDialog::setZTtlControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTtlSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTTL()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid ThatsTheLabor Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTTLpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTTL is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TTL </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTTL</b>";
    ...
    // use mints from zTtl selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTtlControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTtlControlDialog::GetSelectedMints();
    }
    ...
    // Clear zttl selector in case it was used
    ZTtlControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTtl Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTTL </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTTL "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTTL "));
    ui->labelzTTLAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTTLSyncStatus->setVisible(fShow);
}
