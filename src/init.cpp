...
// Copyright (c) 2017 The ThatsTheLabor developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/thatsthelabor-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("thatsthelabor-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "thatsthelabor.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "thatsthelabord.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the TTL and zTTL money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in TTL/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in TTL/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, thatsthelabor, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in TTL/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all ThatsTheLabor specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzttl=<n>", strprintf(_("Enable automatic wallet backups triggered after each zTtl minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizethatsthelaboramount=<n>     " + strprintf(_("Keep N TTL anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The ThatsTheLabor Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("thatsthelabor-loadblk");
...
/** Sanity checks
 *  Ensure that ThatsTheLabor is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ThatsTheLabor Core is shutting down."));
...
#endif
    // Make sure only a single ThatsTheLabor process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ThatsTheLabor Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("ThatsTheLabor version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // ThatsTheLabor: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZTTLMinted();
                        RecalculateZTTLSpent();
                    }
                    RecalculateTTLSupply(1);
                }
                ...
                // ThatsTheLabor: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ThatsTheLabor Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart ThatsTheLabor Core to complete") << "\n";
        ...
        bool fEnableZTtlBackups = GetBoolArg("-backupzttl", true);
        pwalletMain->setZTtlAutoBackups(fEnableZTtlBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeThatsTheLaborAmount = 2;
...
//    }
//
//    nAnonymizeThatsTheLaborAmount = GetArg("-anonymizethatsthelaboramount", 0);
//    if (nAnonymizeThatsTheLaborAmount > 999999) nAnonymizeThatsTheLaborAmount = 999999;
//    if (nAnonymizeThatsTheLaborAmount < 2) nAnonymizeThatsTheLaborAmount = 2;
    ...
    LogPrintf("Anonymize ThatsTheLabor Amount %d\n", nAnonymizeThatsTheLaborAmount);
