...
====================================
This guide will show you how to build thatsthelabord (headless client) for OSX.
...
4.  (Optional) You can also install thatsthelabord to your path:
...
Set the **Build directory** path to the `root` of the clone ThatsTheLabor project. Qt Creator needs this to locate the `Makefile`.
...
To start debugging, locate the following function in `thatsthelabor.cpp` and set a breakpoint.
...
------------------------
You can ignore this section if you are building `thatsthelabord` for your own use.

`thatsthelabord/thatsthelabor-cli` binaries are not included in the `thatsthelabor-Qt.app` bundle.

If you are building `thatsthelabord` or `thatsthelabor-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the ThatsTheLabor-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./thatsthelabord`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./thatsthelabord` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=thatsthelaborrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/ThatsTheLabor/thatsthelabor.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/ThatsTheLabor/thatsthelabor.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/ThatsTheLabor/debug.log

Other commands:
-------

    ./thatsthelabord -daemon # to start the thatsthelabor daemon.
    ./thatsthelabor-cli --help  # for a list of command-line options.
    ./thatsthelabor-cli help    # When the daemon is running, to get a list of RPC commands
