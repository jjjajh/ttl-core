...
*Setup instructions for a gitian build of ThatsTheLabor using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ThatsTheLabor
...
- [Building ThatsTheLabor](#building-thatsthelabor)
...
Clone the git repositories for thatsthelabor and gitian and then checkout the thatsthelabor version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the thatsthelabor repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building ThatsTheLabor
----------------

To build ThatsTheLabor (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the thatsthelabor repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/thatsthelabor/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/thatsthelabor-project/thatsthelabor.git
...
For example:
```bash
URL=https://github.com/thatsthelabor-project/thatsthelabor.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit thatsthelabor=${COMMIT} --url thatsthelabor=${URL} ../ThatsTheLabor/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit thatsthelabor=${COMMIT} --url thatsthelabor=${URL} ../ThatsTheLabor/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit thatsthelabor=${COMMIT} --url thatsthelabor=${URL} ../ThatsTheLabor/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/thatsthelabor-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/thatsthelabor-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/thatsthelabor-build.assert
```
...
[thatsthelabor-project/thatsthelabor/gitian.sigs](https://github.com/thatsthelabor-project/thatsthelabor/gitian.sigs/) repository, or if that's not possible to create a pull
request.
